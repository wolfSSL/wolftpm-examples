/* user_settings.h
 *
 * wolfCrypt benchmark configuration for the MicroBlaze V (RISC-V rv32imc) soft
 * core on the AMD Spartan UltraScale+ SCU35. Standalone wolfCrypt (no wolfTPM):
 * measures raw core crypto throughput on the fabric soft core. Mirrors the
 * ECC-only algorithm set of the fwTPM build (no RSA) so the numbers reflect what
 * the deployed fTPM actually uses, plus the symmetric/hash primitives.
 *
...
/* ---- Single-precision math (portable C, 32-bit) ---- */
...
/* ---- ECC P-256 only (keygen / ECDSA / ECDHE) - matches the deployed fTPM ---- */
#define HAVE_ECC
...
/* ---- Hashing: SHA-1, SHA-256, SHA-3 (matches the deployed fTPM: no SHA-384/512) ---- */
#define WOLFSSL_SHA3
#define HAVE_HKDF
#define HAVE_HMAC
