...
#
# TPM-level NV persistence test for the Mi-V fwTPM (sNVM-backed NV). Defines an
# owner NV index, writes a marker value, and reads it back. Run once to define +
# write; reload the firmware (RAM is wiped, sNVM is not) and run again: the read
# returns the same value and prints PERSISTED, proving the TPM's own NV survived
# in sNVM.
#
